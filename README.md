...
書いたユニットテストを実行しましょう。テストを実行するには `pytest` というライブラリを用いるので、 `requirements.test.txt` というファイルに **テストに必要なライブラリを** 記述しましょう。
...
テスト用に必要なライブラリとアプリケーションを動かすのに必要なライブラリをきちんと分けた方が、どのライブラリがどんな目的で使われているかはっきりするので、スッキリするでしょう。

`pip` コマンドで必要なライブラリをインストールした後に `pytest` コマンドでテストを実行します。
...
### Docker を用いて FastAPI で作成したアプリを起動する
...
- `docker container` コマンドで起動しているコンテナの確認ができます
- `docker stop` コマンドで稼働しているコンテナの停止ができます
- `docker rm` コマンドでコンテナを削除することができます

## ページの HTML ファイルの作成

このセクションでは、ToDo アプリを構成する各ページの HTML ファイルを作成していきます。

以下の 4 つのページを HTML ファイルを用いて作成します。

- ページを遷移知るためのナビゲーションバーを作成する
  o
- タスク一覧ページを作成する
- タスクの詳細ページを作成する
- タスクの新規作成ページを作成する
